...
node = dht.DhtRunner()
my_unique_key = hashlib.md5(os.environ['SPM_USER'].encode('utf-8')).hexdigest() # "cvspiess@ucdavis.edu"
...
def bootstrap():
    ...
    console.print(":sparkles:", f"Putting dict on DHT for [bold blue]{data_dict['name']} ({dht_key})[/bold blue]")
...
def walk_tree_friends(tree_root: Tree, root: str, state: list, depth: int, target_depth: int, friends_visited: set) -> dict:
    ...
    if results:
        ...
        if depth > 1 and root == my_unique_key:
            new_branch = tree_root.add(f"[blue bold] :bust_in_silhouette: {r['name']} [/blue bold]{friendly_depth(0)}")
        else:
...
def main():
    with console.status("[bold blue]Connecting to bootstrap DHT node...", spinner='moon') as status:
    ...
    if args[0] in ('pip', 'poetry'):
        if len(args) > 2 and args[1] == 'install':
            # bootstrap()
            with console.status("[bold blue]Searching social tree...", spinner='moon') as status:
                package = args[2]
                ...
                for r in results:
                    ...
                    for f in friends:
                        ...
                        # print(friend_dict)
            with console.status("[bold blue]Generating social score...", spinner='moon') as status:
            ...
            if question("Are you sure you want to proceed? Y/n"):
                ...
                console.print(":sparkles:", f"Calling 'pip install {package}'")
